...
// Comment is one top-level comment.
type Comment struct {
...
// Comments fetches top-level comments (limit caps how many are returned).
func (c *Client) Comments(ctx context.Context, input string, limit int, sort string) (map[string]any, error) {
	...
	token := findCommentsContinuation(root, sort)
	var comments []Comment
	seen := map[string]bool{}
	for token != "" && len(comments) < limit {
	...
	return map[string]any{"videoId": id, "comments": comments, "count": len(comments), "sort": sortOrDefault(sort)}, nil
}
...
func findCommentsContinuation(root map[string]any, sort string) string {
	var tokens []string
	walkJSON(root, func(key string, val any) bool {
	...
	// Prefer engagement-panel / comments tokens when present.
...
func parseCommentPage(root map[string]any) ([]Comment, string) {
	...
	var next string
	walkJSON(root, func(key string, val any) bool {
		if key == "commentRenderer" || key == "commentEntityPayload" {
			if m, ok := val.(map[string]any); ok {
				...
				if text != "" {
					comments = append(comments, Comment{
						...
						ReplyCount: int(toInt64(nested(m, "replyCount"))),
					})
				}
			}
		}
